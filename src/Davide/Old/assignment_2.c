...
static void print_usage(const char *program) {
    fprintf(stderr,
            ...
            "[--amplitude A] [--threads T] [--scale S] "
            "[--output-prefix PREFIX]\n\n"
            "Defaults are the assignment parameters for student %d. "
            "Output folders are PREFIX1, PREFIX2, and PREFIX3; default PREFIX is 'sim'.\n",
            program, STUDENT_ID);
}
...
static int parse_args(int argc, char **argv, Config *cfg) {
    ...
    if (cfg->gamma < 0.0 || cfg->c <= 0.0 || cfg->dt <= 0.0 || cfg->dx <= 0.0 ||
        ...
        cfg->impulse_j >= cfg->size || cfg->threads < 0 || cfg->pgm_scale < 0.0) {
        return 0;
    }

    if (cfg->pgm_scale == 0.0) {
        cfg->pgm_scale = default_scale(cfg);
    }
...
static int write_pgm(const char *dir, int frame, const double *u, int m,
                     double pgm_scale) {
    char path[512];
    int needed = snprintf(path, sizeof(path), "%s/frame_%05d.pgm", dir, frame);
    ...
    size_t total = (size_t)m * (size_t)m;
    ...
    size_t written = fwrite(pixels, sizeof(unsigned char), total, file);
    free(pixels);
    fclose(file);
...
static int build_scenario(const Config *cfg, int rank, Scenario *scenario) {
    ...
    int needed = snprintf(scenario->output_dir, sizeof(scenario->output_dir),
                          "%s%d", cfg->output_prefix, scenario->id);
...
static int run_scenario(const Config *cfg, const Scenario *scenario, int rank) {
    ...
    for (int frame = 0; frame < cfg->steps; ++frame) {
        ...
        if (!write_pgm(scenario->output_dir, frame, current, cfg->size, cfg->pgm_scale)) {
            ok = 0;
            break;
        }
    ...
    if (ok) {
        printf("Rank %d generated simulation %d in '%s' with %d OpenMP thread(s) in %.3f seconds.\n",
...
int main(int argc, char **argv) {
    ...
    if (num_procs < 3) {
        if (rank == 0) {
            fprintf(stderr, "Part 2 requires at least 3 MPI processes.\n");
        }
        MPI_Finalize();
        return EXIT_FAILURE;
    }
