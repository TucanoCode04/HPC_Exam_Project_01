% Wired into report/report.tex via \input{../src/Davide/Final/Latex/assignment3.tex}
...
regions are rendered white. Student parameters (352165): $\gamma=0.067$,
...
(the same 5-point Laplacian stencil as parts 1--2) is parallelised with
OpenMP \cite{slidestatic}. After each timestep, the rank sends its
...
simulation. Figure~\ref{fig:frame} shows one rendered frame.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.32\textwidth]{../src/Davide/Final/Latex/figures/representative_frame.png}
    \caption{A rendered frame (sim1, late in the simulation): bright extrema, fading fronts, white calm regions.}
    \label{fig:frame}
\end{figure}
...
and \texttt{edu\_sapphire}, are CPU-only). \texttt{edu\_a40} nodes have
2$\times$24-core CPUs with SMT (96 logical cores/node, confirmed via
\texttt{sinfo}'s socket/core/thread fields -- the cluster's own
documentation lists only the 48 physical cores) and 4$\times$A40 GPUs;
...
throughout, and increasingly so: at $M=4096$ it is $\sim$94.6\% of
total time, versus \texttt{compute\_next} at $\sim$2\%. Both phases are
$O(M^2)$, so this ratio stays roughly constant across the whole sweep.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../src/Davide/Final/Latex/figures/size_sweep_execution_time.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../src/Davide/Final/Latex/figures/size_sweep_phase_breakdown.png}
...
far, \texttt{epoll\_wait} ($\sim$52\%) and \texttt{poll} ($\sim$25\%),
most plausibly reflect this rank idling while waiting on the other two
ranks and/or MPI's internal progress engine rather than being tied
directly to each file write; we report them without over-claiming a
precise causal link. To check whether this pattern is an artifact of
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../src/Davide/Final/Latex/figures/nsys_os_runtime_breakdown.png}
...
of threads already saturates available memory bandwidth. This contrasts
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../src/Davide/Final/Latex/figures/thread_sweep_speedup_efficiency.png}
...
$M=4096$: a clear U-shape, worst at both extremes (32 and 1024
threads/block, $\sim$88s average) and best around 128 ($\sim$78s,
$\sim$12\% faster). The same shape was independently reproduced at
$M=1024$ in an earlier sweep, which strengthens confidence this is a
real occupancy effect rather than run-to-run noise.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{../src/Davide/Final/Latex/figures/blocksize_sweep_kernel_time.png}
...
PCIe root complex) but not the deeper one: all three nodes still write
to the same shared network filesystem (BeeGFS/NFS, cluster-wide by
design), so multinode is closer to \texttt{shared} than to
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../src/Davide/Final/Latex/figures/contention_sweep_solo_vs_shared.png}
