...
\begin{equation}
    \resizebox{0.9\textwidth}{!}{$
        u_{i,j}^{t+1} = \frac{1}{\gamma \Delta t + 2} \left[ c^2 \Delta t^2 \left( \frac{u_{i+1,j}^t - 2u_{i,j}^t + u_{i-1,j}^t}{\Delta x^2} + \frac{u_{i,j+1}^t - 2u_{i,j}^t + u_{i,j-1}^t}{\Delta x^2} \right) + \gamma u_{i,j}^t \Delta t - 2u_{i,j}^{t-1} + 4u_{i,j}^t \right]
    $}
\end{equation}
...
and $t=32$ is roughly halved (from $-19.3$ to $-10.0$ percentage points
