...
$u_{i,j}^{t}$. The derivatives in the equation are approximated with \textbf{central differences method}, so that the next wave state $u_{i,j}^{t+1}$ is:
\begin{equation}
    \resizebox{0.9\textwidth}{!}{$
        u_{i,j}^{t+1} = \frac{1}{\gamma \Delta t + 1} \left[ c^2 \Delta t^2 \left( \frac{u_{i+1,j}^t - 2u_{i,j}^t + u_{i-1,j}^t}{\Delta x^2} + \frac{u_{i,j+1}^t - 2u_{i,j}^t + u_{i,j-1}^t}{\Delta x^2} \right) + \gamma u_{i,j}^t \Delta t - u_{i,j}^{t-1} + 2u_{i,j}^t \right]
    $}
\end{equation}
...
All profiling and testing in this section was performed using the \texttt{Edu\_skylake} partition (2 nodes, 32 physical and logical cores) \cite{hpcguide} with 32 threads
...
outperforms \texttt{schedule(static)} in practice \footnote{See output\_1920835.txt (static) and output\_1920842.txt (guided)}: 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../assignment_1/plots/timeandCPUU96.png}
...
As $M$ increases ($M=5000$ to $15000$), the computational density grows quadratically ($O(M^2)$) and in accordance with Gustafson's Law, the useful computational work begins to outweigh the fixed parallelization overhead, driving the CPU utilization upward.
